...
plot(dvar2, cv.tot.store, type="l")
points(dvar2, cv.tot.store, pch=21, col="white", bg="white", cex=2)
points(dvar2, cv.tot.store, pch=21, col="black", bg="white", cex=1)

matplot(dvar2, cv.tot.store, type="l", lwd=2, col=c("black", "grey35", "grey50", "grey75"),
        xlim=c(0,2.5), ylim=c(0.02,0.12),
        lty="solid", ylab="Coefficient of Variation", 
        xlab="Demographic Stochasticity of Dominant Species",
        cex.lab=0.8, cex.axis=0.8)
...
labs = c(expression(paste(beta[i1], " = 0.1")),
         ...
         expression(paste(beta[i1], " = 0.5")),
         expression(paste(beta[i1], " = 0.7")))
text(c(2.3), y=c((cv.tot.store[10,1]-0.002), cv.tot.store[10,2], 
                   cv.tot.store[10,3], cv.tot.store[10,4]),
     labs, cex=0.8)
