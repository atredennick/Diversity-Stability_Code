...
Rpars=list(intcpt.mu=rep(0,Nspp),intcpt.yr=matrix(0,Nyrs,Nspp),intcpt.tau=rep(100,Nspp),
  intcpt.gr=matrix(NA,2,Nspp),g.tau=rep(NA,Nspp),
  dd=matrix(NA,Nspp,Nspp),theta=rep(NA,Nspp),sizeMean=rep(NA,Nspp),sizeVar=rep(NA,Nspp),
  recSizes=list(1))

 infile="recruitment/recruit_params.csv"    
...
#  Rdata=Rdata[-tmp,]
 tmp=paste("Rpars$",Rdata$X,"<-",Rdata[,2],sep="")
 eval(parse(n=dim(Rdata)[1],text=tmp))
 for(i in 1:Nspp){
   infile=paste("../../Data/Kansas/",sppList[i],"/recSize.csv",sep="")
...
f=function(v,u,Rpars,rpa,doSpp) { 
	  ...
	  tmp=dnorm(v,Rpars$sizeMean[doSpp],sqrt(Rpars$sizeVar[doSpp]))/
      (1-pnorm(-1.61,Rpars$sizeMean[doSpp],sqrt(Rpars$sizeVar[doSpp])))
