...
#the species' yearly intrinsic growth rate (r[yr] in Lotka-Volterra terms)


# ATT 8/26/14
# outfile1="ipm_cover_yearlyGrowth.csv"
...
maxSize<-c(3000,900,3000)    # in cm^2: PSSP=225 HECO=202  POSE=260  ARTR=3000  # minSize=0.2  cm^2
Nyrs=34
...
for(jjjj in 1:length(sppList)){
  ...
  make.K.values=function(v,u,muWG,muWS, #state variables
                         Rpars,rpa,Gpars,Spars,doYear,doSpp,doGroup){  #growth arguments
    f(v,u,Rpars,rpa,doSpp)+S(u,muWS,Spars,doYear,doSpp,doGroup)*G(v,u,muWG,Gpars,doYear,doSpp,doGroup) 
  }
  
  # Function to make iteration matrix based only on mean crowding
  make.K.matrix=function(v,muWG,muWS,Rpars,rpa,Gpars,Spars,doYear,doSpp,doGroup) {
    muWG=expandW(v,v,muWG)
    muWS=expandW(v,v,muWS)
    
    K.matrix=outer(v,v,make.K.values,muWG,muWS,Rpars,rpa,Gpars,Spars,doYear,doSpp,doGroup)
  ...
  for (i in 2:(tlimit)){
    ...
    cover=covSave[i-1,3:5]; N=Nsave[i-1,]
    rpa=get.rpa(Rpars,cover,doYear,doGroup)
    ...
    for(doSpp in 1:Nspp){  
      if(cover[doSpp]>0){    
        ...
        K.matrix=make.K.matrix(v[[doSpp]],WmatG[[doSpp]],WmatS[[doSpp]],Rpars,rpa,Gpars,Spars,doYear,doSpp,doGroup)  
...
# 
colnames(maxR) <- sppList
saveRDS(maxR, "./simulations/RmaxYearly_ipm.rds")
# outfile <- "./simulations/RmaxYearly_ipm.csv"
# output <- as.data.frame(maxR)
# colnames(output) <- sppList
# output$yearParams <- c(1:22)
