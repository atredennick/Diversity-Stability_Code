model{
  for(i in 1:Nobs){
    mu[i] <- intcpt.yr[yr[i]] + intG[grp[i]] + intI[plant[i]] + slope.yr[yr[i]]*x[i] + inprod(NBbeta.mu[1:Nspp],crowd[i,])
  ...
  for(p in 1:Nplants){
    intI[p]~dnorm(0,tauPlant)
  }
  tauPlant~dgamma(0.001, 0.001)
  ...
  for(m in 1:Nspp){
    NBbeta.mu[m]~dnorm(0,NBbeta.tau) T(-3,-0.000001) ##change this to meet BUGS(too wide to fail)
  }
