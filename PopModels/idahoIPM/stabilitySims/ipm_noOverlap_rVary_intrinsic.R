...
invade=FALSE #FALSE for intrinsic growth rate; TRUE for invasion growth rate

ifelse(invade==TRUE,
       outfile <- "./stabilitySims/invasionGrowthRates_YrFluct.csv",
       outfile <- "./stabilitySims/intrinsicGrowthRates_YrFluct.csv")
...
rSims <- read.csv("./stabilitySims/randomOffsets_rNEW.csv")  #random offsets for intercepts in vital rate regressions
yrVector <- readRDS("./stabilitySims/yrVector.rds") #get vector of random years (same across mono and mixture simulations)
maxR <- matrix(nrow = nrow(rSims), ncol = length(sppList))
...
for(jjjj in 1:length(sppList)){
  ...
  for(perturbNow in 1:nrow(rSims)){
    rScale <- as.numeric(rSims[perturbNow,])
    
    Gpars$intcpt <- Gpars$intcpt*rScale
    Spars$intcpt <- Spars$intcpt*rScale
    Rpars$intcpt.yr <- (Rpars$intcpt.yr-Rpars$intcpt.mu)+(Rpars$intcpt.mu*rScale)
    ...
    maxR[perturbNow,jjjj] <- pgrMean
    
    #   ## Write ouput==============================================================
    #   outfile1=paste("ipm_pgr_max_",fixSpp,".csv",sep="")
    #   output=as.data.frame(covSave)
    #   output=subset(output,time>burn.in) ###here is the place to let resident species approach stable...
    #   write.table(output,outfile1,row.names=F,sep=",")
  }#end perturbation (community) loop
}#end species loop
