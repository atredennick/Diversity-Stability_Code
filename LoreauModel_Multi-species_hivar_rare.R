...
# 
# matplot(seq(1,500), two.species[,1:32], type="l")
# 
# mean.b <- numeric(32)
# for(i in 1:32){
#   mean.b[i] <- mean(two.species[,i])
# }
# 
# df.rank <- data.frame(abund = sort(mean.b, decreasing=TRUE),
#                       rank = seq(1,32))
# 
# library(ggplot2)
# 
# ggplot(data=df.rank) + geom_line(aes(rank, abund), color="grey50") +
#   geom_point(aes(rank, abund), size=4, color="white") +
#   geom_point(aes(rank, abund), size=3, color="grey20") +
#   theme_bw()
# 

# 
cv = cv[complete.cases(cv),]
write.csv(cv, "stability_simulations_dom_hivar_rare.csv")
